...
| 🧪 **Skills** | `skills/<slug>/SKILL.md`를 first-class 자원으로 다룹니다 — Anthropic Agent Skills 표준 + ai-barracks 확장(`aib_version`, `upstream`). v1.1부터 모든 배럭이 council seed를 자동 보유. |
...
| `aib skills [list\|doctor]` | 스킬 목록 조회 및 무결성 점검 (v1.1+) |
| `aib hook {start,end,continue}` | (내부) CLI hook에서 자동 호출 |
| `aib version` | 버전 확인 |

### `aib skills list`

배럭의 `skills/<slug>/SKILL.md`를 walk해 등록된 스킬과 frontmatter 메타데이터를 출력. v1.1+.
...
각 SKILL.md의 frontmatter 무결성을 점검 (필수 필드 누락 / yaml 손상). v1.1+.
